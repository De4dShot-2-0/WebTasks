'use strict';
var info = [{ ratingRevievs: "264 отзыва", price: { oldUan: "4 333 грн", newUan: "3 799 грн" }, name: "Motorola MOTO G4 (XT1622) Black" },
...
let sectCatch = document.getElementById('endPage');
document.body.insertBefore(section, sectCatch);
...
for (let el in info) {
    ...
    nameBox.append(info[el].name);
    prodBox.appendChild(nameBox);

    if (info[el].price.length) {
        let priceBox = document.createElement('div');
        priceBox.className = "priceBox";
        priceBox.append(info[el].price);
        prodBox.appendChild(priceBox);
    } else {
        let newPriceBox = document.createElement('div');
        let oldPriceBox = document.createElement('div');
        newPriceBox.className = "newPriceBox";
        oldPriceBox.className = "oldPriceBox";
        oldPriceBox.append(info[el].price.oldUan);
        newPriceBox.append(info[el].price.newUan);
        prodBox.appendChild(oldPriceBox);
        prodBox.appendChild(newPriceBox);
    }

    let rateBox = document.createElement('div');
    rateBox.className = "ratingBox";
    rateBox.append("Всего " + info[el].ratingRevievs);
    prodBox.appendChild(rateBox);

    section.appendChild(prodBox);
}

function sortByPrice(info) {
    info.sort((a, b) => a.priceSort > b.priceSort ? 1 : -1)
}

function addSortParam(info) {
    for (let el in info) {
        if (info[el].price.length) {
            let num = Number(info[el].price.substr(0, info[el].price.length - 4).replace(/\s/, ""));
            info[el]['priceSort'] = num;
        } else {
            let num = Number(info[el].price.newUan.substr(0, info[el].price.newUan.length - 4).replace(/\s/, ""));
            info[el]['priceSort'] = num;
        }
    }
}